...
* `projectile-root-marked` looks for `.projectile` (or whatever you've set as the value of `projectile-dirconfig-file`) just like `projectile-root-bottom-up`. The idea is that normally if you have a `.projectile` file you'd like it to override the normal project root discovery logic.
...
* `projectile-root-bottom-up-recurring` will look for project markers that can appear at every level of a project (e.g. `Makefile` or `.svn`) and will return the bottom-most match (closest to the current folder) that does not also have a marker in its parent directory. This is analogous to `projectile-root-bottom-up` but with recurring marker files.
